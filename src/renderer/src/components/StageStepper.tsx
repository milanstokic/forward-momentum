...
/** The shared horizontal pipeline shown in the top bar. */
export function StageStepper({ stages }: { stages: PipelineStage[] }): JSX.Element {
  return (
      ...
      {stages.map((st, i) => (
        <Fragment key={st.key}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontSize: 10.5, color: stepColor[st.status], whiteSpace: 'nowrap' }}>
              {st.name}
            </span>
          </div>
          {i < stages.length - 1 && (
